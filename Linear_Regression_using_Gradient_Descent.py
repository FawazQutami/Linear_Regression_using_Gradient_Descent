...
class LinearRegressionGd:
    ...
    def __init__(self, alpha=0.01, n_iterations=100):
        """   Constructor  """
        # Learning Rate
        self.alpha = alpha
    ...
    def fit(self, x_trn, y_trn):
        """ Fitting data and minimize the cost function using Gradient Descent """

        # Initialize the parameters
        n_samples, n_features = x_trn.shape
        self.weight = np.zeros(n_features)  # 1 feature
        self.bias = 0
        ...
        for iteration in range(self.iters):
            ...
            y_predicted = np.dot(x_trn, self.weight) + self.bias
            ...
            """ 
                To minimize cost we use Gradient Descent to
                calculate the partial derivatives of the cost
                function:
            """
            ...
            """
            self.weight -= self.alpha * weight_derivative
            ...
            if iteration % 1 == 0:
                ...
                for i in range(len(self.weight)):
                    tempDict['W' + str(i)] = self.weight[i]
    ...
    def predict(self, x_tst):
        ...
        y_predicted = np.dot(x_tst, self.weight) + self.bias

        return y_predicted


def draw_plot(xs, predicted_line, x_trn, x_tst,
              y_trn, y_tst):
    """
    """
...
def programmed_lr(X, X_train, X_test, y_train, y_test):
    """"""
    start = time.time()

    # Create a Linear Regression object
    reg = LinearRegressionGd(alpha=0.5, n_iterations=1000)
...
if __name__ == '__main__':
    try:
        ...
        programmed_lr(X, X_train, X_test, y_train, y_test)
